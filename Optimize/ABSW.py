...
class OptApplication(QApplication, QGuiApplication):
    def setWidgets(self,
                   displayName : Optional[str] = None,
                   icon : Union[str, QIcon, None] = None,
                   attribute: Qt.ApplicationAttribute = Qt.AA_EnableHighDpiScaling
                   ) -> "OptApplication":
        self.setApplicationDisplayName(displayName)
        self.setAttribute(attribute, True)
        if icon:
            self.setWindowIcon(icon if isinstance(icon, QIcon) else QIcon(icon))
        return self

__all__ = ['OptApplication', 'AbstractWidget']